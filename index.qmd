...
![](images/clipboard-2226049323.gif)

## Vad kan man göra i R?

Förutom att hantera och analysera data så är R väldigt bra för visualisering

![](images/clipboard-2279219876.png)

## Följ med i presentationen

Du kan kopiera koden i presentationen genom och klicka i högra hörnet kodblocket

Hjälptext finns ibland tillagt
...
1.  Detta är en hjälptext
2.  Det här är också en hjälptext
...
Tidigare fanns inte `|>` i base R. då krävdes paketet magrittr och såg ut såhär `%>%`
...
library(skimr)
skimr::skim(datafil) #<1>

```

1.  "skimr::" betyder att man väljer funktionen skim() från paketet skimr. Är användbart då flera paket kan använda samma namn \## Ta ut en tabell

Finns användbara paket för att ta ut tabeller

```{r}

#install.packages(gt) # <1>
#install.packages(gtsummary) # <1>

library(gt)
library(gtsummary)

datafil |> tbl_summary() # <2>

```

1.  Om inte tidigare installerat. Ta bort \# för att installerat paketen
2.  Om ingenting specificeras så väljs alla variabler i tabellen

## Ta ut en tabell

Man kan specificera variabler genom `include=c()`

```{r}
datafil |> tbl_summary(by=sex, include=c(age, hospital)) 
```

## Snygga till tabellen

Man kan ändra olika specifikationer

```{r}
datafil |> 
  tbl_summary(by=sex, include=c(age, hospital),
              statistic = list( #<1>
                all_continuous() ~ "{median} ({p25}, {p75}), (n={N_nonmiss})",
                all_categorical() ~ "{n}/{N} ({p}%)"),
              digits = c(all_categorical() ~ c(0, 0, 1), #<2>
                         all_continuous() ~ c(1, 0, 0)))  
```

1.  Här specificeras vad som syns i rutorna. "{n}/{N} ({p}%)" blir 3/6 (50.0%) i tabellen
2.  Här specificeras antal decimaler. Om specifikationen hade varit "all_categorical() \~ c(1, 1, 2)" hade tabellen visat 3.0/6.0 (50.00%)

## Snygga till tabellen

Även storlek och dylikt kan specificeras
...
datafil |> 
  tbl_summary(by = sex, include = c(age, hospital),
              ...
              all_continuous() ~ "{median} ({p25}, {p75}), (n={N_nonmiss})",
                all_categorical() ~ "{n}/{N} ({p}%)"),
              digits = c(all_categorical() ~ c(0, 0, 1),
                         all_continuous() ~ c(1, 0, 0))) |>
  modify_header(
    label = "*Variabel*",
    stat_1 = "**Kvinnor**  \nN = {n}",
  ...
  modify_spanning_header(c(stat_1, stat_2) ~ "Tabell uppdelat på kön")|>
  as_gt() |>
...
datafil |> 
  ...
  gt::gtsave("~/Downloads/tabell_forskarskolan.docx") # <1> 
```

1.  funktionen gtsave från pakete gt sparar ner tabellen som word dokument

## Spara tabell

Man kan också spara en enkel tabell och sedan manuellt i word

```{r}
datafil |> tbl_summary(by=sex, include=c(age, hospital)) |> 
  as_gt() |> 
  gt::gtsave("~/Downloads/enkel_tabell_forskarskolan.docx") 
```
